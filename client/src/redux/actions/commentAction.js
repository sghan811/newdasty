...
import { POST_TYPES, ALL_TYPES, COM_TYPES } from "./postAction";
...
export const createCommentAll = ({ post, newComment, auth, socket }) => async (
  dispatch
) => {
  const newPost = { ...post, comments: [...post.comments, newComment] };
  ...
  try {
    ...
    // todo socket
    socket.emit("createCommentAll", newPost);
...
export const createCommentCom = ({ post, newComment, auth, socket }) => async (
  dispatch
) => {
  const newPost = { ...post, comments: [...post.comments, newComment] };
  ...
  try {
    ...
    // todo socket
    socket.emit("createCommentCom", newPost);
...
export const deleteCommentAll = ({ post, comment, auth, socket }) => async (
  dispatch
) => {
  const deleteArr = [
    ...post.comments.filter((cm) => cm.reply === comment._id),
  ...
  socket.emit("deleteCommentAll", newPost);
