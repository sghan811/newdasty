...
export const PROFILE_TYPES = {
  ...
  GET_BOSTS: "GET_PROFILE_BOSTS",
  UPDATE_POST: "UPDATE_PROFILE_POSTS",
  UPDATE_BOST: "UPDATE_PROFILE_BOSTS",
};
...
export const getProfileUsers = ({ id, auth }) => async (dispatch) => {
  ...
  try {
    ...
    const res2 = getDataAPI(`/user_bosts/${id}`, auth.token);

    const users = await res;
    const posts = await res1;
    const bosts = await res2;
    ...
    dispatch({
      type: PROFILE_TYPES.GET_BOSTS,
      payload: { ...bosts.data, _id: id, page: 2 },
    });
