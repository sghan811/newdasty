...
import { deleteCom, reportPost } from "../../../redux/actions/postAction";
import { IoPeopleCircleOutline } from "react-icons/io5";

const CardHeaderComuCom = ({ post, bost }) => {
  ...
  const handleDeletePost = () => {
    if (window.confirm("Are you sure?")) {
      dispatch(deleteCom({ post, auth, socket }));
      return history.push("/");
    }
  };
    ...
    <div className="card_header cardpad">
      <div className="d-flex">
        <div className="medium-avatar-cover">
          {bost.images[0] ? (
            <Avatar src={bost.images[0].url} size="medium-avatar" />
          ) : (
            <IoPeopleCircleOutline className="medium-avatar header-community" />
          )}
          {/* <Avatar src={bost.images[0].url} size="medium-avatar" /> */}
...
export default CardHeaderComuCom;
